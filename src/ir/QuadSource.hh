...
class QuadSource {
  ...
  std::string to_string() const {
    ...
    if (is_value()) {
      return fmt::format("QuadSource [{}]", as_value().to_string());
    }
    
    if (is_function()) {
      return fmt::format("QuadSource [{}]", Operand{as_function()}.to_string());
    }
  ...
  T get_as() const {
    if (!is_a<T>()) {
      std::string error_message = fmt::format("Invalid get_as<{}>() on held type \"{}\"\n", get_typename<T>(), get_held_variant_name());
